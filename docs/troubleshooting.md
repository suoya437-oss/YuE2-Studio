...
### 22. Seed-VC 首次运行卡死：模型下载连接黑洞
现象：日志停在「步骤 2/4」，GPU 0%，inference.py 进程活着但无报错、无下载进度，可挂数小时。根因：HF CDN 连接建立后读停滞（黑洞），无读超时兜底。另注意：inference.py 把模型缓存到**仓库内 `./checkpoints`**（相对 seed-vc 目录），不是 `hf-seedvc/`——排查时别盯错目录。
**修**：杀掉卡死进程后重试即可（hf_hub_download 断点续传，重跑通常直接命中缓存）。彻底预防：模型预下已纳入 `bin/preinstall_envs.sh`（超时+续传+5 次重试）。
